-- This schema create script runs on every application load
-- Script failures will cause the application to fail to start
-- MySQL compatible schema
CREATE TABLE IF NOT EXISTS PERSONAL (
  ALTERNATIVE_ID varchar(100) NOT NULL,
  PERCENTILE float NOT NULL DEFAULT 0.0,
  SAT_VERBAL integer NOT NULL DEFAULT 0,
  SAT_MATH integer NOT NULL DEFAULT 0,
  ACT_COMPOSITE integer NOT NULL DEFAULT 0,
  AGE integer NOT NULL DEFAULT 1,
  RACE integer,
  GENDER integer,
  STATUS integer NOT NULL DEFAULT 1,
  EARNED_CREDIT_HOURS integer NOT NULL DEFAULT 0,
  GPA_CUMULATIVE float,
  GPA_SEMESTER float,
  STANDING integer NOT NULL DEFAULT 1,
  PELL_STATUS boolean DEFAULT FALSE,
  CLASS_CODE varchar(2),
  ENROLLMENT_STATUS varchar(100),
  PRIMARY KEY (ALTERNATIVE_ID)
);

-- COURSE_ID,SUBJECT,ENROLLMENT,ONLINE_FLAG
CREATE TABLE IF NOT EXISTS COURSE (
  COURSE_ID varchar(100) NOT NULL,
  SUBJECT varchar(55),
  ENROLLMENT integer DEFAULT 0,
  ONLINE_FLAG boolean DEFAULT false,
  PRIMARY KEY (COURSE_ID)
);

-- ALTERNATIVE_ID,COURSE_ID,FINAL_GRADE,WITHDRAWAL_DATE
CREATE TABLE IF NOT EXISTS ENROLLMENT (
  ALTERNATIVE_ID varchar(100) NOT NULL,
  COURSE_ID varchar(100) NOT NULL,
  FINAL_GRADE varchar(10),
  WITHDRAWAL_DATE TIMESTAMP,
  PRIMARY KEY (ALTERNATIVE_ID,COURSE_ID),
  FOREIGN KEY (ALTERNATIVE_ID) REFERENCES PERSONAL(ALTERNATIVE_ID),
  FOREIGN KEY (COURSE_ID) REFERENCES COURSE(COURSE_ID)
);

-- ALTERNATIVE_ID,COURSE_ID,GRADABLE_OBJECT,CATEGORY,MAX_POINTS,EARNED_POINTS,WEIGHT,GRADE_DATE
CREATE TABLE IF NOT EXISTS GRADE (
  ALTERNATIVE_ID varchar(100) NOT NULL,
  COURSE_ID varchar(100) NOT NULL,
  GRADABLE_OBJECT varchar(250) NOT NULL,
  CATEGORY varchar(255),
  MAX_POINTS float DEFAULT 0.0,
  EARNED_POINTS float NOT NULL DEFAULT 0.0,
  WEIGHT float,
  GRADE_DATE TIMESTAMP,
  PRIMARY KEY (ALTERNATIVE_ID,COURSE_ID,GRADABLE_OBJECT),
  FOREIGN KEY (ALTERNATIVE_ID) REFERENCES PERSONAL(ALTERNATIVE_ID),
  FOREIGN KEY (COURSE_ID) REFERENCES COURSE(COURSE_ID)
);

-- ALTERNATIVE_ID,COURSE_ID,EVENT,EVENT_DATE
CREATE TABLE IF NOT EXISTS ACTIVITY (
  ID integer NOT NULL AUTO_INCREMENT,
  ALTERNATIVE_ID varchar(100) NOT NULL,
  COURSE_ID varchar(100) NOT NULL,
  EVENT varchar(255) NOT NULL,
  EVENT_DATE TIMESTAMP NOT NULL,
  PRIMARY KEY (ID),
  FOREIGN KEY (ALTERNATIVE_ID) REFERENCES PERSONAL(ALTERNATIVE_ID),
  FOREIGN KEY (COURSE_ID) REFERENCES COURSE(COURSE_ID)
);

CREATE TABLE IF NOT EXISTS CONFIGURATION (
  ID integer NOT NULL AUTO_INCREMENT,
  SSP_BASE_URL varchar(100) NOT NULL,
  SSP_RISK_CONFIDENCE_THRESHOLD float NOT NULL,
  SSP_ACTIVE boolean DEFAULT true,
  PRIMARY KEY (ID)
);
